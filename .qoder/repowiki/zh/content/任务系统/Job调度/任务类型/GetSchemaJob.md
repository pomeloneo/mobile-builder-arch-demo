...
<cite>
**本文引用的文件**
- [get-schema-job.ts](file://packages/h5-builder/src/jobs/get-schema-job.ts) - *已更新以集成 PrefetchService*
- [prefetch.service.ts](file://packages/h5-builder/src/services/prefetch.service.ts) - *新增的预加载服务*
- [lifecycle.ts](file://packages/h5-builder/src/jobs/lifecycle.ts) - *已更新生命周期阶段名称*
...
## 更新摘要
**变更内容**
- 更新 GetSchemaJob 以集成新的 PrefetchService，现在在获取页面 Schema 后立即调用 `startPrefetch` 启动后台数据预取。
- 将生命周期阶段 `PageLifecycle.LoadComponentLogic` 重命名为 `LoadComponentLogicAndPrefetch` 以更准确地反映其职责。
- 新增 PrefetchService 服务类，负责管理预加载任务的并发控制、优先级调整和数据缓存。
- 更新架构图以反映新的数据流和组件交互。
...
## 简介
本文档围绕 GetSchemaJob 的实现进行深入解析，重点说明其作为任务系统起点的职责：通过依赖注入获取 SchemaService 和 PrefetchService 实例，在页面生命周期的“Open”阶段拉取页面 Schema 数据，并在获取后立即启动后台数据预取。文档详细描述了该任务如何继承 AbstractJob 并重写 prepare 方法，通过依赖注入获取服务实例发起异步请求。重点更新了与 PrefetchService 的集成方式，包括预取请求的触发、并发控制、优先级调整和错误处理。提供了实际代码示例展示如何配置预取任务和处理响应数据。说明了该任务在任务调度器中的前置依赖角色，以及输出 Schema 数据和预取数据如何被后续 BuildTreeJob 和组件模型消费。记录了常见问题如网络失败时的降级策略和缓存机制。

## 项目结构
GetSchemaJob 位于任务系统模块中，与生命周期、组件服务、Schema 服务、预取服务、依赖注入标识符等共同构成前端页面构建流水线的关键节点。
...
G["PrefetchService<br/>数据预取服务"]
H["Service Identifiers<br/>DI 标识符"]
end
subgraph "基础设施"
I["Barrier<br/>屏障/同步点"]
J["Demo Data<br/>演示 Schema 数据"]
end
A --> B
B --> E
B --> F
...
**图表来源**
- [get-schema-job.ts](file://packages/h5-builder/src/jobs/get-schema-job.ts#L1-L130)
...
- [schema.service.ts](file://packages/h5-builder/src/services/schema.service.ts#L1-L54)
...
## 核心组件
- GetSchemaJob：继承自 AbstractJob，作为任务系统的起点，负责在 Open 生命周期中拉取 Schema 并注册组件异步加载器，随后开启组件资源预加载和数据预取。
...
- PrefetchService：提供数据预取功能，支持并发控制、优先级调整和数据缓存；GetSchemaJob 通过它启动后台数据预取。
...
- [get-schema-job.ts](file://packages/h5-builder/src/jobs/get-schema-job.ts#L1-L130)
- [abstract-job.ts](file://packages/h5-builder/src/bedrock/launch/abstract-job.ts#L1-L46)
- [barrier.ts](file://packages/h5-builder/src/bedrock/async/barrier.ts#L1-L40)
- [lifecycle.ts](file://packages/h5-builder/src/jobs/lifecycle.ts#L1-L18)
- [component.service.ts](file://packages/h5-builder/src/services/component.service.ts#L1-L797)
- [schema.service.ts](file://packages/h5-builder/src/services/schema.service.ts#L1-L54)
- [prefetch.service.ts](file://packages/h5-builder/src/services/prefetch.service.ts#L1-L273)
...
- 依赖注入：通过 DI 标识符注入 SchemaService、ComponentService 和 PrefetchService。
- 生命周期：在 Open 阶段执行，拉取 Schema 并注册组件异步加载器。
- 资源预加载：调用 ComponentService.preloadComponentsUnified(schema)，并使用 Barrier 确保后续任务等待资源就绪。
- 数据预取：调用 PrefetchService.startPrefetch(schema.prefetch, schema.root) 启动后台数据预取。
...
participant PrefetchSvc as "PrefetchService"
...
Job->>PrefetchSvc : startPrefetch()<br/>启动数据预取
...
- [schema.service.ts](file://packages/h5-builder/src/services/schema.service.ts#L1-L54)
- [component.service.ts](file://packages/h5-builder/src/services/component.service.ts#L620-L797)
- [prefetch.service.ts](file://packages/h5-builder/src/services/prefetch.service.ts#L1-L273)
- [barrier.ts](file://packages/h5-builder/src/bedrock/async/barrier.ts#L1-L40)
...
- 依赖注入：通过装饰器注入 SchemaService、ComponentService 和 PrefetchService，使用 DI 标识符进行解析。
- 生命周期执行：在 Open 阶段执行，拉取 Schema 并注册组件异步加载器，随后预加载组件资源并通过 Barrier 开放。
- 输出数据：通过 SchemaService.getSchema() 为后续任务提供可用的 Schema 数据，并通过 PrefetchService 为组件模型提供预取数据。
- 新增功能：在获取 Schema 后立即调用 PrefetchService.startPrefetch 启动后台数据预取。
...
GetSchemaJob --> PrefetchService : "依赖注入"
```
...
- 生命周期阶段 `PageLifecycle.LoadComponentLogic` 已重命名为 `LoadComponentLogicAndPrefetch` 以更准确地反映其职责。
...
- [component.service.ts](file://packages/h5-builder/src/services/component.service.ts#L279-L797)

**章节来源**
- [get-schema-job.ts](file://packages/h5-builder/src/jobs/get-schema-job.ts#L69-L116)
- [component.service.ts](file://packages/h5-builder/src/services/component.service.ts#L279-L797)

### 与 PrefetchService 的集成
- GetSchemaJob 在获取 Schema 后立即调用 PrefetchService.startPrefetch 启动后台数据预取。
- PrefetchService 根据 Schema 中的预取配置（prefetch）发起异步请求，获取组件所需的数据。
- 预取任务是异步的，不阻塞主流程，但后续组件模型可以通过 PrefetchService.getData 获取预取数据。
- PrefetchService 支持并发控制（最大并发数6）、优先级调整（根据 Tab 激活状态）和错误处理（捕获错误，避免中断其他任务）。

```mermaid
sequenceDiagram
participant GSJ as "GetSchemaJob"
participant PFS as "PrefetchService"
participant Schema as "Schema"
GSJ->>PFS : startPrefetch(prefetchConfigs, rootSchema)
PFS->>PFS : executePrefetch()
PFS->>PFS : sortByPriority()
PFS->>PFS : processWithConcurrency()
PFS->>PFS : fetchData()
PFS-->>GSJ : 返回
```

**图表来源**
- [get-schema-job.ts](file://packages/h5-builder/src/jobs/get-schema-job.ts#L59-L66)
- [prefetch.service.ts](file://packages/h5-builder/src/services/prefetch.service.ts#L28-L103)
...
- PrefetchService 的数据获取方法（fetchData）目前是 Mock 实现，可以轻松替换为真实接口调用。

**章节来源**
- [schema.service.ts](file://packages/h5-builder/src/services/schema.service.ts#L23-L32)
- [service-identifiers.ts](file://packages/h5-builder/src/services/service-identifiers.ts#L1-L20)
- [prefetch.service.ts](file://packages/h5-builder/src/services/prefetch.service.ts#L159-L178)
...
- PrefetchService 的错误处理：在 processWithConcurrency 中捕获每个预取任务的错误，避免单个任务失败中断整个预取流程。
...
- [prefetch.service.ts](file://packages/h5-builder/src/services/prefetch.service.ts#L245-L271)
...
- 在 PrefetchService 中替换 fetchData() 实现以接入真实接口调用，示例路径参考 [prefetch.service.ts](file://packages/h5-builder/src/services/prefetch.service.ts#L159-L178)。
...
## 依赖分析
- GetSchemaJob 依赖 SchemaService、ComponentService 和 PrefetchService，三者通过 DI 标识符注入。
- BuildTreeJob 同样依赖 SchemaService 和 ComponentService，且在 Prepare 阶段读取 Schema。
...
- 缓存机制：ComponentService 内置 Model 与 View 缓存，避免重复加载；SchemaService 内置 Schema 缓存，避免重复拉取；PrefetchService 内置预取数据缓存，避免重复请求。
- PrefetchService 支持并发控制（最大并发数6）和优先级调整（根据 Tab 激活状态），优化资源利用率。

**章节来源**
- [component.service.ts](file://packages/h5-builder/src/services/component.service.ts#L620-L797)
- [schema.service.ts](file://packages/h5-builder/src/services/schema.service.ts#L1-L54)
- [prefetch.service.ts](file://packages/h5-builder/src/services/prefetch.service.ts#L13-L14)
...
## 结论
GetSchemaJob 作为任务系统的起点，承担了拉取页面 Schema、注册组件异步加载器、预加载组件资源和启动数据预取的核心职责。通过与 SchemaService、ComponentService 和 PrefetchService 的协作，以及 Barrier 的同步机制，确保后续 BuildTreeJob 能在 Schema 与组件资源就绪的前提下安全执行。当前实现以演示数据为主，若需接入真实 HTTP 接口，可在 SchemaService 中扩展 fetchSchema() 并结合 HttpService 的请求参数构造、超时与重试机制，配合错误降级与缓存策略，进一步提升稳定性与性能。新增的 PrefetchService 为组件提供了后台数据预取能力，支持并发控制、优先级调整和错误处理，显著提升了用户体验和页面性能。